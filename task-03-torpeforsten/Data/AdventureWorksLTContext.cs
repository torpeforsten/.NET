...
public partial class AdventureWorksLTContext : DbContext
{
    ...
    // t2
    public async Task<decimal> ProductCategoryItemsListPriceAverageAsync(int categoryId)
    {
        // Ensure the DbSet for your Product entity is named correctly
        var products = await Products
            .Where(p => p.ProductCategoryId == categoryId)
            .ToListAsync();

        // If products found, calculate the average list price
        if (products.Any())
        {
            // Ensure the ListPrice property is named correctly
            decimal averageListPrice = products.Average(p => p.ListPrice);
    ...
    {
        // Ensure the DbSet for your Address entity is named correctly
        var addresses = await Set<Address>()
            .Where(a => a.CustomerAddresses.Any(ca => ca.Customer.CompanyName.ToLower() == companyName.ToLower()))
            .ToListAsync();
    ...
    {
        // Ensure the DbSet for your ProductCategory entity is named correctly
        var categories = await Set<ProductCategory>()
            .OrderBy(c => c.ProductCategoryId)
            .ToListAsync();

        var result = new List<string>();
        // Loop through the categories and sub-categories
        foreach (var category in categories.Where(c => c.ParentProductCategoryId == null))
        {
            // Main category
            result.Add($"**{category.ProductCategoryId}: {category.Name}**");

            // Sub-categories
            var subCategories = categories.Where(c => c.ParentProductCategoryId == category.ProductCategoryId);
            // Loop through the sub-categories
            foreach (var subCategory in subCategories)
            {
                result.Add($"{category.ProductCategoryId}.{subCategory.ProductCategoryId}: {subCategory.Name}");
            }
        }
...
{
    // Ensure the DbSet for your Customer entity is named correctly
    var customer = await Set<Customer>()
    ...
    // Find the address of the specified type
    var customerAddress = customer.CustomerAddresses
        .FirstOrDefault(ca => ca.AddressType.ToLower() == addressType.ToLower());

    if (customerAddress != null)
    {
        // Populate address properties
        customerInfo.AddressLine1 = customerAddress.Address.AddressLine1;
        customerInfo.City = customerAddress.Address.City;
...
/*
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
                ...
                // Configuration logic here, e.g., setting the connection string
